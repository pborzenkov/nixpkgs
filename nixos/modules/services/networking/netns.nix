  ...
  namespaces = mkOption {
    ...
    type = types.attrsOf (types.submodule {
      options.nftables = mkOption {
        description = mdDoc "Nftables ruleset within the network namespace.";
        type = types.lines;
      ...
      options.sysctl = options.boot.kernel.sysctl // {
        description = mdDoc "sysctl within the network namespace.";
        default = config.boot.kernel.sysctl;
        defaultText = literalMD "config.boot.kernel.sysctl";
      };
      options.service = mkOption {
        description = mdDoc "Systemd configuration specific to this netns service";
        type = types.attrs;
        default = {};
      };
    });
